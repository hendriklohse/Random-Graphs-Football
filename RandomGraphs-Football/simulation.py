...
x_grid_size = n
y_grid_size = n
# MAKE SURE TO SET THESE EQUAL. This is n.
...
def create_graph(P, L, T_R, T_K, D, grid):
    # T_K = 7.58 #mean
    ...
    while time < endTime:
        ...
        newStart = False
        # print(startNodeID, startPosition)
        # print("entering for loop now")
        for nodeID, positions in grid:
            # print(nodeID, positions['pos'][0], positions['pos'][1])
            # print(endPosition_coords)
            if newStart:
                # print("newStart was true")
                break
            # if nodeID == x_grid_size*y_grid_size + 1:
            #     #goal against, reset
            #     if (positions['pos'][0] - 1 / (20 * x_grid_size)) < endPosition_coords[0] <= (positions['pos'][0] + 1 / (20 * x_grid_size)):
            #         # print("first if normal case")
            #         if (positions['pos'][1] - 1 / (20 * x_grid_size)) < endPosition_coords[1] <= (positions['pos'][1] + 1 / (20 * x_grid_size)):
            ...
            #             reset = True
            #             break
            # if nodeID == x_grid_size*y_grid_size + 2:
            #     #goal for, reset
            #     if (positions['pos'][0] - 1 / (20 * x_grid_size)) < endPosition_coords[0] <= (positions['pos'][0] + 1 / (20 * x_grid_size)):
            #         # print("first if normal case")
            #         if (positions['pos'][1] - 1 / (20 * x_grid_size)) < endPosition_coords[1] <= (positions['pos'][1] + 1 / (20 * x_grid_size)):
            ...
            #             reset = True
            #             break
            if positions['pos'][0] > 0 and positions['pos'][1] > 0 and positions['pos'][0] < ((x_grid_size - 1) / x_grid_size) and positions['pos'][1] < ((x_grid_size - 1) / x_grid_size):
                # print("entering normal case")
                if (positions['pos'][0] - 1 / (2 * x_grid_size)) < endPosition_coords[0] <= (positions['pos'][0] + 1 / (2 * x_grid_size)):
                    # print("first if normal case")
                    if (positions['pos'][1] - 1 / (2 * x_grid_size)) < endPosition_coords[1] <= (positions['pos'][1] + 1 / (2 * x_grid_size)):
                        # print("second if normal case")
                        all_edges.append(str((str(startNodeID), str(nodeID))))
                ...
                if endPosition_coords[0] <= (positions['pos'][0] + 1 / (2 * x_grid_size)):
                    ...
                    if endPosition_coords[1] <= (positions['pos'][1] + 1/(2 * x_grid_size)):
                        ...
                        newStart = True
                        break
            elif positions['pos'][0] >= ((x_grid_size - 1) / x_grid_size) and positions['pos'][1] >= ((x_grid_size - 1) / x_grid_size):
                ...
                if endPosition_coords[0] >= (positions['pos'][0] + 1 / (2 * x_grid_size)):
                    ...
                    if endPosition_coords[1] >= (positions['pos'][1] + 1 / (2 * x_grid_size)):
                        ...
                        newStart = True
                        break
            elif positions['pos'][0] >= ((x_grid_size - 1) / x_grid_size) and 0 < positions['pos'][1] < ((x_grid_size - 1) / x_grid_size):
                ...
                if (positions['pos'][0] - 1 / (2 * x_grid_size)) < endPosition_coords[0]:
                    ...
                    if (positions['pos'][1] - 1 / (2 * x_grid_size)) < endPosition_coords[1] <= (positions['pos'][1] + 1 / (2 * x_grid_size)):
                        ...
                        newStart = True
                        break
            elif positions['pos'][1] >= ((x_grid_size - 1) / x_grid_size) and 0 < positions['pos'][0] < ((x_grid_size - 1) / x_grid_size):
                ...
                break
                # print("own goal case, ignore")
            # print("now at end of for loop")
...
for i in tqdm(range(nr_graphs)):
    all_graphs.append(create_graph(P = 432.44, L = 125.74, T_R = 11.95, T_K = 7.58, D = 0.2486, grid=grid))


G = make_average_graph(all_graphs, VISUALIZE=False)
print(G)
nx.write_adjlist(G, "G_" + str(nr_graphs) + "_n_" + str(x_grid_size) + ".adjlist")
